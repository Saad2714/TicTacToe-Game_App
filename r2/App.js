...
drawItem = itemNumber => {
  ...
  if (itemArray[itemNumber] === 'empty') {
    ...
    //  Changing turn ...ab saamnewalee ki baari. so icon change (type)
...
chooseItemIcon = itemNumber => {
  ...
  if (itemArray[itemNumber] !== "empty") {
    return(
      itemArray[itemNumber] ? "cross" : "circle"
    )
  } 
  return "code"
}
...
if((itemArray[0] !== "empty") && (itemArray[0] == itemArray[1]) && (itemArray[1] == itemArray[2]) ){ 
  ...
  this.setState({winMessage: (itemArray[0] ? "Cross" : "Circle").concat("Wins... Aayega to Patel hi!! " ) }) 
}

else if((itemArray[1] != "empty") && (itemArray[1] == itemArray[4]) && (itemArray[4] == itemArray[7]) ){ 
  this.setState({winMessage: (itemArray[1] ? "Cross" : "Circle").concat("Wins... Aayega to Patel hi!! " ) }) 
}

else if((itemArray[2] != "empty") && (itemArray[2] == itemArray[5]) && (itemArray[5] == itemArray[8]) ){ 
  this.setState({winMessage: (itemArray[2] ? "Cross" : "Circle").concat("Wins... Aayega to Patel hi!! " ) }) 
}

else if((itemArray[3] != "empty") && (itemArray[3] == itemArray[4]) && (itemArray[4] == itemArray[5]) ){ 
  this.setState({winMessage: (itemArray[3] ? "Cross" : "Circle").concat("Wins... Aayega to Patel hi!! " ) }) 
}

else if((itemArray[6] != "empty") && (itemArray[6] == itemArray[7]) && (itemArray[7] == itemArray[8]) ){ 
  this.setState({winMessage: (itemArray[6] ? "Cross" : "Circle").concat("Wins... Aayega to Patel hi!! " ) }) 
}
...
render(){
  return (
    <View style={styles.container}>
          <Text style={styles.welcomeMessage}>Hello Saad, Welcome to Anchor_Games!!!</Text>
